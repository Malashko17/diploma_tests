...
class TestProfileSettings:
    ...
    @allure.title("Проверка на правильное отображение ранее введеных данных")
    @pytest.mark.profile_settings
    def test_the_correct_user_data_is_displayed(self, driver, login):
    ...
    @allure.severity(allure.severity_level.NORMAL)
    @allure.title("Добавление дополнительной информации о пользователе")
    @pytest.mark.profile_settings
    def test_add_additional_user_info(self, driver, login):
        profile_settings_page = ProfileSettings(driver)
        profile_settings_page.open_profile_setting_page()
    ...
    @allure.severity(allure.severity_level.NORMAL)
    @allure.title("Изменение пароля на невалидное значение")
    @pytest.mark.profile_settings
    def test_change_incorrect_password(self, driver, login):
        profile_settings_page = ProfileSettings(driver)
        profile_settings_page.open_profile_setting_page()
